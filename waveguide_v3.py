...
# 三组螺旋线
@gf.cell
def three_spiral(wg_width=0.5, st_length=10000, io_gap=500, group_id=1):
    ...
    for i in range(1, 4):
        ...
        t_spiral << gf.geometry.boolean(
            gf_spiral_t.extract(layers=gf.LAYER.WGCLAD),
            gf_spiral_t.extract(layers=gf.LAYER.WG),
            "A-B",
        )
        # 两边直波导参数设置
        wg_length1 = st_length/2
        wg_length2 = st_length/2-port_spacing
        waveguide_temp_1 = gf.components.straight(length=wg_length1)
        ...
        t_spiral << waveguide_temp_1
        t_spiral << waveguide_temp_2
        # taper参数设置
        ...
        t_spiral << text_temp_1
        t_spiral << text_temp_2
        t_spiral << text_temp_3
...
waveguide.write_gds("waveguide_v3.gds")
